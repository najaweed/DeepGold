...
class DeepAR(nn.Module):
    def __init__(self, input_size, device, output_size=1, encoder_len=1,
        ...
        if (encoder == None and decoder == None):
            assert (
                        hidden_size != None and num_layers != None), "Using LSTM, but hidden_size and num_layers not specified!"
            ...
            assert (
                        rnn_hidden_size != None and rnn_layers != None), "Using custom encoder and decoder without specifying rnn sizes!"
    ...
    def forward(self, input, target, hidden_encoder=None,
        ...
        if (self.in_train_mode):
            ...
            for idx in range(self.decoder_len):
                ...
                self.decoder.flatten_parameters()
                output_decoder, (hidden_decoder, cell_decoder) = self.decoder(decoder_input,
                                                                              (hidden_decoder, cell_decoder))
